...
describe('meeting PDF export', () => {
  it('builds a meeting note PDF filename and plain-text content', () => {
    const prep = getMeetingPrep(workbase, 'meeting_brampton', '2026-05-07')
    ...
    expect(pdfExport.fileName).toBe('sundesk-meeting-note-brampton-prep-2026-05-15.pdf')
    expect(pdfExport.text).toContain('Sundesk meeting note')
    expect(pdfExport.text).toContain('Brampton prep.')
